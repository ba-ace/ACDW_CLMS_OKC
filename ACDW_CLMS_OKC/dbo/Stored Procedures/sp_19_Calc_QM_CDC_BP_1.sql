create procedure [sp_19_Calc_QM_CDC_BP]
@years int,
@LOB varchar(20)
as
...
select distinct subscriber_id, seq_claim_id  from adw.[tvf_get_claims_w_dates]('Diastolic Less Than 80','Diastolic 80–89' ,'',concat('1/1/',@year), concat('12/31/',@year) ) 
intersect  
select distinct subscriber_id, seq_claim_id from adw.[tvf_get_claims_w_dates]('Systolic Less Than 140','','',concat('1/1/',@year), concat('12/31/',@year) ) 
) firsts join (
...
select distinct subscriber_id, seq_claim_id,primary_svc_date  from adw.[tvf_get_claims_w_dates]('Diastolic Less Than 80','Diastolic 80–89' ,'Diastolic Greater Than/Equal To 90',concat('1/1/',@year), concat('12/31/',@year) ) 
union  
select distinct subscriber_id, seq_claim_id,primary_svc_date  from adw.[tvf_get_claims_w_dates]('Systolic Less Than 140','Systolic Greater Than/Equal To 140','',concat('1/1/',@year), concat('12/31/',@year) ) )
b ) c where c.rank =1 ) seconds on firsts.SEQ_CLAIM_ID = seconds.SEQ_CLAIM_ID
--has to be outpatient or nonacute 
join (select distinct seq_claim_id from adw.[tvf_get_claims_w_dates]('Outpatient','Nonacute Inpatient' ,'',concat('1/1/',@year), concat('12/31/',@year) ) ) thirds on firsts.SEQ_CLAIM_ID = thirds.SEQ_CLAIM_ID
