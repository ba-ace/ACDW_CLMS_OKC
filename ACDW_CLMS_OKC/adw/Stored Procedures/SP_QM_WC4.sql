CREATE PROCEDURE adw.[SP_QM_WC4]
AS
     IF OBJECT_ID('tmp_QM_WC4_NUM_T', 'U') IS NOT NULL
         DROP TABLE tmp_QM_WC4_NUM_T;
     CREATE TABLE tmp_QM_WC4_NUM_T(member VARCHAR(50));
     INSERT INTO tmp_QM_WC4_NUM_T
            SELECT DISTINCT 
                   subscriber_id
            FROM
            (
            ...
            WHERE A.num_of_wc_vis = 4;
     IF OBJECT_ID('tmp_QM_WC4_NUM', 'U') IS NOT NULL
         DROP TABLE tmp_QM_WC4_NUM;
     CREATE TABLE tmp_QM_WC4_NUM(member VARCHAR(50));

     --meas year screening by professional
     INSERT INTO tmp_QM_WC4_NUM
            SELECT DISTINCT 
                   *
            FROM tmp_QM_WC4_NUM_T
            INTERSECT
            SELECT DISTINCT 
                   *
            FROM tmp_QM_WC0_DEN;
